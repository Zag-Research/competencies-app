-- The real competency list for the studio: 40 for CPS109, 40 for CPS213.
--
-- THIS FILE IS PRODUCTION DATA. It is the course content, transcribed from the lists
-- Dave and Jonathan wrote, and it is what a real deployment loads on top of schema.sql.
-- It contains no people.
--
-- Split out of seed.sql (#92), which mixes sample students, sample results and sample
-- coverage pairs in with these. Loading that file into production would enrol five
-- students who do not exist and credit TAs with evaluations they never did, silently.
--
--   production:   schema.sql  then  competencies.sql  then the real roster
--   development:  schema.sql  then  competencies.sql  then seed.sql
